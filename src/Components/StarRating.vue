<!-- From: https://github.com/JonathanDn/vue-stars-rating/blob/master/star-rating.vue -->
<!-- Copying because need to make small tweaks. Primarily the #gradient id format by adding _uid -->
<template>
  <div class="star-rating" :aria-label="rating + ' of 5'">
    <div v-for="(star, index) in stars" :key="index" class="star-container">
      <svg
        class="star-svg"
        :style="[
          { fill: `url(#gradient${star.raw}-${_uid})` },
      ...
      >
        ...
        <defs>
          <!--
					...
					-->
          <linearGradient :id="`gradient${star.raw}-${_uid}`">
            <stop
              id="stop1"
              :offset="star.percent"
              stop-opacity="1"
